...
# NOTE: logfire.configure() is intentionally NOT called here at module level.
# When this module is imported by the main FastAPI app, logfire is already
# configured by app/main.py. Calling configure() again here would override the
# app's token-authenticated logfire session. Configuration only happens below
# in the __main__ block when running this script standalone.
...
if __name__ == "__main__":
    ...
    load_dotenv()
    # Configure logfire with token when running standalone (not imported by FastAPI app)
    logfire.configure(
